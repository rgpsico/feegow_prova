...
function listar_profissional(id = null) { 
   var action = 'professional/list?especialidade_id=' + id;
    $.getJSON(url, {
        ...
        console.log(especialidade.content);
          $('.count-prof').text(' ');
       ...
       var imagem             = (especialidade.content[a].foto == null ? "assets/img/img_avatar1.png" : especialidade.content[a].foto);
                ...
                div += "<p class='nome_profi'>" + Utils.toLimit(Nome_profissional , ) + "</p>";
...
$('body').on('click', '.solicitar', function () {
  ...
  if(nome == '' || cpf == ''){
    ...
    $.post("controller/crud.php", prod, function (data) {
           $.alert({
            boxWidth:'500px',
            title:"Sucesso!",
            useBootstrap: false,
